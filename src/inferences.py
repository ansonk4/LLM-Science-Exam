...
# first prediction using deberta for all questions
def first_prediction(test_df, device: str):
    test_predictions = []
...
# second prediction using mistral for the 12.5% hardest questions
def second_prediction(test_predictions, test_df, device: str):
    ...
    return pred_2, worst_question_ids

def main():
    device = 'cuda'
    pred1= first_prediction(test_df_2, device)

    gc.collect()
    torch.cuda.empty_cache()

    pred2, worst_question_ids = second_prediction(pred1, test_df_2, device)

    predictions_as_ids = np.argsort(-pred2)[:,:3]
    predictions_as_answer_letters = np.array(list('ABCDE'))[predictions_as_ids]
    predictions_as_string = [
        ' '.join(row) for row in predictions_as_answer_letters
    ]
